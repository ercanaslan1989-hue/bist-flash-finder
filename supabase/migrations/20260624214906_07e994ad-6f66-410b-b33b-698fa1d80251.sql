...
DECLARE
  _db int;
BEGIN
  -- 1) single-pass: day_index, returns, volume ratios, value columns
  WITH calc AS (
    SELECT ds.id,
  ...
  -- 3) capture pre-event features at 1,2,3,5,10 trading days before
  DELETE FROM public.event_features;
